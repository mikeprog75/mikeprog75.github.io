    ...
    <body>
	  ...
	  <header>
            ...
            <section>
	            ...
	            <div id="popup">
				<p>Sur cette page, vous pouvez publier votre tutoriel. <br /><br />
				   Les règles à suivre sont les suivantes: <br /><br />
				   1- Vous devez être inscrit sur le site avant de publier un tutoriel <br />
				   2- La discipline et le titre doivent être en rapport avec votre tutoriel <br />
				   3- Ne publiez pas un cours sur le piratage informatique, la magie ou la sorcelerie <br />
...
<?php
  if (empty($_POST['pseudo']) && empty($_POST['titre_tutoriel']) && empty($_POST['tutoriel']))
  {
    ...
    echo '<form method="post" action="tutoriel_pc.php"  name="formulaire">
	...
	$tutoriel=$_POST['tutoriel'];
			 ...
			 if (!preg_match("#^[A-Z]{1}+[a-z0-9.-\s]+$#", $titre_tutoriel) || empty($titre_tutoriel))
			 {
			   ...
			   $titre_tutoriel_erreur2 = "Ce titre existe déjà ";
			   $i++;
			   
			   }
			   
			   
			    // on verifie si le tuto a été saisi
			 if (empty($_POST['tutoriel']))
			 {
			   ...
			   if ($i==0)
               {
                                  ...
                                  else
					              {
					                 ...
					                 echo "Votre tutoriel a été posté";
									 
							// Insertion du message à l'aide d'une requête préparée
             $req = $bdd->prepare('INSERT INTO tutoriels ( pseudo, discipline, titre_tutoriel, tutoriel, date_tutoriel ) VALUES( :pseudo, :discipline, :titre_tutoriel, :tutoriel, NOW() )');
    ...
    else
    {
		...
		echo'<p>Cliquez <a href="./tutoriel_pc.php">ici</a> pour recommencer</p>';
