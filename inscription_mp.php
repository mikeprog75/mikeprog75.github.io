...
<?php
if ( empty($_POST['nom']) && empty($_POST['prenom']) && empty($_POST['pseudo']) && empty($_POST['pass'])  && empty($_POST['confirm']) &&
   ...
   
{
	echo '<div id = obli>' . 'Vous devez remplir tout ce qui est obligatoire' .  '</div>' ;
	  echo '<form  method="post" action="inscription_mp.php"  enctype="multipart/form-data" >
	  <div id="formulaire">
	  <fieldset>
	  <legend>Formulaire d inscription</legend>
        ...
        <div id="LastName"><label>Nom :</label></div> <input type="text" size="25" id="nom" name="nom" maxlength="15" /><br />
		
		<div id="firstName"><label>Prénom :</label></div> <input type="text" size="25" id="prenom" name="prenom" maxlength="15"  /><br />	
	...
	<label for=" "></label><textarea cols="40" rows="4" name = "loisirs" id="loisirs">Parlez de vos loisirs (sport, musique, cinema...) en quelques lignes (facultatif)</textarea><br />
	
	
	
	<div id="send"><input class="blue-button-x" type="submit" value="S inscrire" /> <input class="red-button-x" type="reset" value="Réinitialiser le formulaire" /></div>
	</fieldset>
	</div>
	</form>';
...
<?php
    ...
    $confirm = sha1('gz'.$_POST['confirm']);
			 ...
			 // on verifie si le nom saisi est en majuscule
			 if (!preg_match("#^[A-Z.-\s]+$#", $nom) || empty($nom))
			 {
			 
			    $nom_erreur = "Veuillez saisir un nom en majuscule";
			    $i++;
				
				}
				
				// on verifie si le prénom saisi est valide
			   if (!preg_match("#^[A-Z]{1}[a-z.-\s]+$#", $prenom) || empty($prenom))
			   {
			   
			      $prenom_erreur = "Saisir un prénom dont la première lettre est en majuscule et le reste en miniscule";
			      $i++;
			   ...
			   $pseudo_erreur2 = "Un membre possède déjà ce pseudo";
			   $i++;
	...
	if ($i==0)
   {
	echo'<h1>Inscription terminée</h1>';
        ...
        echo'<p>Bienvenue '.stripslashes(htmlspecialchars($_POST['pseudo'])).' vous êtes maintenant inscrit sur le site' . '<br /><br />'. 
	   ...
	   $nom_avatar=(!empty($_FILES['avatar']['size']))?move_avatar($_FILES['avatar']):''; 
        ...
        /*S'il n'y a aucun message d'erreur alors, on enregistre le nouveau membre*/
        $req=$bdd->prepare('INSERT INTO forum_membres(sexe, nom, prenom, pseudo, nom_avatar, pass , email, localisation, loisirs, date_inscription) 
		VALUES(:sexe, :nom, :prenom, :pseudo, :nom_avatar, :pass, :email, :localisation, :loisirs, NOW() )');
        $req->execute(array( 'sexe'=>htmlspecialchars($sexe), 'nom'=>htmlspecialchars($nom), 'prenom'=>htmlspecialchars($prenom), 'pseudo'=>htmlspecialchars($pseudo), 
	    'nom_avatar'=>htmlspecialchars($nom_avatar), 'pass'=>htmlspecialchars($pass), 'email'=>htmlspecialchars($email), 
		'localisation'=>htmlspecialchars($localisation), 'loisirs'=>htmlspecialchars($loisirs) ));
    ...
    else
    {
        ...
        echo'<p>Une ou plusieurs erreurs se sont produites lors de l inscription</p>';
        echo'<p>'.$i.' erreur(s)</p>';
		echo'<p>'.$nom_erreur.'</p>';
		...
		echo'<p>Cliquez <a href="./inscription_mp.php">ici</a> pour recommencer</p>';
